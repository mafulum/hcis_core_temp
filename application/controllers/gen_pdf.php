...
.bio-graph-heading table tr td{
			  ...
			  <div class="panel-body bio-graph-info">
				  ...
				  <br />' . $this->global_m->get_array_data($data['emp_map'], "PERSH") . '</p>
					  ...
					  <div class="bio-row">
						  ...
						  <p><span>Tgl. Pensiun </span>: ' . $this->global_m->get_array_data($data['empDate'], "TTanggalPensiun") . ' </p>
						  ...
						  <p><span>Service Year in Current Position </span>: ' . $this->global_m->get_array_data($data['svcyear'], "SVCY") . 'years ' . $this->global_m->get_array_data($data['svcyear'], "SVCM") . 'months</p>
						  <br />
						  <h1>Last Education</h1>
						  ';
        for ($i = 0; $i < count($data['lastEduc']); $i++) {
            $html.='<p>' . $this->global_m->get_array_data($data['lastEduc'][$i], "STEXT") . ', ' . $this->global_m->get_array_data($data['lastEduc'][$i], "SLTP1") . ', ' . $this->global_m->get_array_data($data['lastEduc'][$i], "INSTI") . ", " . $this->global_m->get_array_data($data['lastEduc'][$i], "EMARK") . ", " . $this->global_m->get_array_data($data['lastEduc'][$i], "LULUS");
            if ($this->global_m->get_array_data($data['lastEduc'][$i], "BIAYA") != 'Pribadi') {
                $html.=", " . $this->global_m->get_array_data($data['lastEduc'][$i], "BIAYA") . '</p>';
            }
        }
        $html .='<br />
						  <br />                                                  
        ...
        if ($data['future']) {
            ...
            $html.="<p><i>Please see Profile Matchup Report</i></p>";
        $html.='<br /><br />
        ...
        if ($data['employee']) {
            ...
            foreach ($data['employee'] as $emp) {
                $html.='<tr>
                                            ...
                                            <td>' . $this->global_m->get_array_data($emp, "currpos") . '<br/>' . $this->global_m->get_array_data($emp, "perusahaan") . '</td>
...
// OUTPUT
    ...
    function srankm($sHash) {
        $str = $this->get_download($sHash);
        $data = json_decode($str, true);
//        var_dump($data);
//        echo "<br/>";
        $data['externalCSS'] = '<link rel="stylesheet" type="text/css" href="' . base_url() . 'css/profile.css" />';
...
.space{
	padding: 5px;
        ...
        if ($data['employee']) {
            $j = 1;
            foreach ($data['employee'] as $emp) {
                if(empty($emp['readiness']))$emp['readiness']="00";
                if(empty($emp['cperf']))$emp['cperf']="00";
                if(empty($emp['cage']))$emp['cage']="00";
                if(empty($emp['ceduc']))$emp['ceduc']="00";
                if(empty($emp['cmedical']))$emp['cmedical']="00";
                $html.='<tr>
                                            <td>' . $j . '</td>
                                            <td><img name="poto" width="60px" height="60px" alt="" src="' . base_url() . 'img/photo/' . $this->global_m->get_array_data($emp, "nik") . '.jpg" onerror="this.src=' . base_url() . 'img/photo/default.jpg"></td>
                                            <td>' . $this->global_m->get_array_data($emp, "nama") . '
                                                <br/>' . $this->global_m->get_array_data($emp, "nik") . '
                                            </td>
                                            <td>' . $this->global_m->get_array_data($emp, "currpos") . '<br/>' . $this->global_m->get_array_data($emp, "perusahaan") . '</td>
                                            <td>' . $this->global_m->get_array_data($emp, "mdg") . '</td>
                                            <td style=" background-image: -moz-linear-gradient(left, #5cb85c ' . number_format($this->global_m->get_array_data_num($emp, "readiness"),0) . '%, #ffffff ' . number_format($this->global_m->get_array_data_num($emp, "readiness"),0) . '%, #ffffff);">' . number_format($this->global_m->get_array_data_num($emp, "readiness"),2) . '% '.$this->global_m->get_array_data($emp, "ready").'</td>
                                            <td style=" background-image: -moz-linear-gradient(left, #5cb85c ' . number_format($this->global_m->get_array_data_num($emp, "ccompt"),0) . '%, #ffffff ' . number_format($this->global_m->get_array_data_num($emp, "ccompt"),0) . '%, #ffffff);">' . number_format($this->global_m->get_array_data_num($emp, "ccompt"),2) . '%</td>
                                            <td style=" background-image: -moz-linear-gradient(left, #5cb85c ' . number_format($this->global_m->get_array_data_num($emp, "cperf"),0) . '%, #ffffff ' . number_format($this->global_m->get_array_data_num($emp, "readiness"),0) . '%, #ffffff);">' . number_format($this->global_m->get_array_data_num($emp, "cperf"),2) . '%</td>
                                            <td style=" background-image: -moz-linear-gradient(left, #5cb85c ' . number_format($this->global_m->get_array_data_num($emp, "cage"),0) . '%, #ffffff ' . number_format($this->global_m->get_array_data_num($emp, "cage"),0) . '%, #ffffff);">' . number_format($this->global_m->get_array_data_num($emp, "cage"),2) . '%</td>
                                            <td style=" background-image: -moz-linear-gradient(left, #5cb85c ' . number_format($this->global_m->get_array_data_num($emp, "ceduc"),0) . '%, #ffffff ' . number_format($this->global_m->get_array_data_num($emp, "ceduc"),0) . '%, #ffffff);">' . number_format($this->global_m->get_array_data_num($emp, "ceduc"),2) . '%</td>
                                            <td style=" background-image: -moz-linear-gradient(left, #5cb85c ' . number_format($this->global_m->get_array_data_num($emp, "cmedical"),0) . '%, #ffffff ' . number_format($this->global_m->get_array_data_num($emp, "cmedical"),0) . '%, #ffffff);">' . number_format($this->global_m->get_array_data_num($emp, "cmedical"),2) . '%</td>
                                            <td>' . $this->global_m->get_array_data($emp, "mapdesc") . ' (' . $this->global_m->get_array_data($emp, "map") . ')</td>
                                        </tr>';
                $j++;
            }
        } else {
