        ...
        $data['scriptJS'] = '<script>
				...
				function confirmUpdPos(){
					...
					$.post("' . base_url() . 'index.php/orgchart/update_pos",{iPosition:xiPosition,iParentPos:xiParentPos,
                                            cPosShort:xcPosShort,begda:dPosBegda,endda:dPosEndda,cPosText:xcPosText,cPosPriox:xcPosPriox},function(result){                                             
                                            blockPage(result);
                ...
                if ($aSSubs) {
                    foreach ($aSSubs as $idx1 => $aSSub) {
                        ...
                        $aData[$idx1]['name'] = '<span style="color:#1CAADC"><i class="fa fa-dot-circle-o"></i> ' . $aSSub['STEXT'] . '</span>' .
                                ' <div class="tree-actions"><a href="#" onClick="posEdit(\'E\',\'' . $aSSub['OBJID'] . '\',\'' . $aSSub['STEXT'] . '\',\'' . $aSSub['SHORT'] . '\',\'' . $this->global_m->convert_yyyymmdd_ddmmyyyy($aSSub['BEGDA']) . '\',\'' . $this->global_m->convert_yyyymmdd_ddmmyyyy($aSSub['ENDDA']) . '\',\'' . $aSSub['PRIOX'] . '\');">' .
