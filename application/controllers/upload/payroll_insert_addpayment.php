 ...
 * @author Garuda
 */
class payroll_insert_addpayment extends CI_Controller {

    //put your code here
    function __construct() {
        parent::__construct();
        $this->load->model('pa_payroll/emp_addpayment_m');
    }

    function index() {
        $data = $this->emp_addpayment_m->reff_upload_page();
    ...
    function upload() {
        ...
        if (!empty($_FILES['userfile']) && isset($_FILES['userfile']['name']) && !empty($_FILES['userfile']['name'])) {
            ...
            if (empty($sError)) {
                $filename = "m_emp_additional_" . date("Ymd_Hi") . '.xls';
                //copy file
    ...
    function load($filename) {
        ...
        $header = 1;
        $col = 1;
        $aStringHeader = array("PERNR","CNAME","BEGDA","WGTYP", "EVTDA", "WAMNT","WAPNT","NOTE");
        ...
        for ($i = 2; $i <= $MAX_ROW; $i++) {
            ...
            $aData['WGTYP'] = $this->reader->sheets[0]['cells'][$i][4];
            $aData['EVTDA'] = null;
            ...
            if(isset($this->reader->sheets[0]['cells'][$i][7])){
                $aData['WAPNT'] = $this->reader->sheets[0]['cells'][$i][7];
            }
            $aData['NOTE'] = null;
            ...
            //pengecekan error 
            $aMandatory = array("PERNR","BEGDA","WGTYP");
            ...
            if(empty($aData['WAMNT']) && empty($aData['WAPNT'])){
        ...
        for ($i = 0; $i < count($aInput); $i++) {
            $this->emp_addpayment_m->emp_addpayment_new($aInput[$i]);
            $sReturn.="<tr>";
            foreach($aStringHeader as $header){
