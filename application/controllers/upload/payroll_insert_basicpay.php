 ...
 * @author Garuda
 */
class payroll_insert_basicpay extends CI_Controller {
    ...
    function index() {
        $data = $this->basicpay_m->reff_upload_page();
    ...
    function upload() {
        ...
        if (!empty($_FILES['userfile']) && isset($_FILES['userfile']['name']) && !empty($_FILES['userfile']['name'])) {
            ...
            if (empty($sError)) {
                $filename = "m_emp_basicpay_" . date("Ymd_Hi") . '.xls';
                //copy file
    ...
    function load($filename) {
        ...
        $header = 1;
        $col = 1;
        $aStringHeader = array("PERNR","CNAME","BEGDA","ENDDA","WGTYP", "WAMNT","NOTE");
        ...
        for ($i = 2; $i <= $MAX_ROW; $i++) {
            ...
            if(isset($this->reader->sheets[0]['cells'][$i][7])){
                $aData['NOTE'] = addslashes($this->reader->sheets[0]['cells'][$i][7]);
            }
            
            //pengecekan error 
            $aMandatory = array("PERNR","CNAME","BEGDA","ENDDA","WAMNT","WGTYP");
            ...
            if(empty($aWGTYP[$aData['WGTYP']])){
                $aWGTYP[$aData['WGTYP']] = $this->global_m->getMasterBasicPayScaleSingle($aData['WGTYP']);
            }
            if(empty($aWGTYP[$aData['WGTYP']])){
                $sError.=$aData['WGTYP']." NOT FOUND @ row $i<br/>";
            }
            
            if(empty($aData['WAMNT'])){
                $sError.=" WAMNT can't empty @row $i<br/>";                
            }
        ...
        for ($i = 0; $i < count($aInput); $i++) {
            $this->basicpay_m->personal_basicpay_new($aInput[$i]);
            $sReturn.="<tr>";
            foreach($aStringHeader as $header){
