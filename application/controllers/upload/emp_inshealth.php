 ...
 * @author Garuda
 */
class emp_inshealth extends CI_Controller {
    ...
    function index() {
        $data = $this->inshealth_m->reff_upload_page();
    ...
    function upload() {
        ...
        if (!empty($_FILES['userfile']) && isset($_FILES['userfile']['name']) && !empty($_FILES['userfile']['name'])) {
            ...
            if (empty($sError)) {
                $filename = "m_emp_inshealth_" . date("Ymd_Hi") . '.xls';
                //copy file
    ...
    function load($filename) {
        ...
        $header = 1;
        $col = 1;
        $aStringHeader = array("PERNR","BEGDA","ENDDA","INSTY", "INSID", "RDATE","FAMSA","OBJPS","NOTE");
        ...
        for ($i = 2; $i <= $MAX_ROW; $i++) {
            ...
            $aData['INSID'] = $this->reader->sheets[0]['cells'][$i][5];
            $aData['RDATE'] = null;
            if(isset($this->reader->sheets[0]['cells'][$i][6])){
                $aData['RDATE'] = $this->reader->sheets[0]['cells'][$i][6];
            }
            $aData['FAMSA'] = $this->reader->sheets[0]['cells'][$i][7];
            $aData['OBJPS'] = null;
            if(isset($this->reader->sheets[0]['cells'][$i][8])){
                $aData['OBJPS'] = addslashes($this->reader->sheets[0]['cells'][$i][8]);
            }
            $aData['NOTE'] = null;
            ...
            //pengecekan error 
            $aMandatory = array("PERNR","BEGDA","ENDDA","INSTY","INSID","FAMSA");
        ...
        for ($i = 0; $i < count($aInput); $i++) {
            $this->inshealth_m->personal_inshealth_new($aInput[$i]);
            $sReturn.="<tr>";
            foreach($aStringHeader as $header){
