 ...
 * @author Garuda
 */
class emp_montask extends CI_Controller {
    ...
    function index() {
        $data = $this->emp_motask_m->reff_upload_page();
    ...
    function upload() {
        ...
        if (!empty($_FILES['userfile']) && isset($_FILES['userfile']['name']) && !empty($_FILES['userfile']['name'])) {
            ...
            if (empty($sError)) {
                $filename = "m_emp_montask_" . date("Ymd_Hi") . '.xls';
                //copy file
    ...
    function load($filename) {
        ...
        $header = 1;
        $col = 1;
        $aStringHeader = array("PERNR","BEGDA","ENDDA","REMINDER_DATE", "REMINDER_TYPE", "PERNR_OLD","SK_NO","NOTE");
...
//        }
        ...
        for ($i = 2; $i <= $MAX_ROW; $i++) {
            ...
            $aData['REMINDER_DATE'] = $this->reader->sheets[0]['cells'][$i][4];
            $aData['REMINDER_TYPE'] = str_pad($this->reader->sheets[0]['cells'][$i][5],4,"0",STR_PAD_LEFT);
            $aData['PERNR_OLD'] = null;
            if(isset($this->reader->sheets[0]['cells'][$i][6])){
                $aData['PERNR_OLD'] = $this->reader->sheets[0]['cells'][$i][6];
            }
            $aData['SK_NO'] = null;
            if(isset($this->reader->sheets[0]['cells'][$i][7])){
                $aData['SK_NO'] = addslashes($this->reader->sheets[0]['cells'][$i][7]);
            }
            $aData['NOTE'] = null;
            if(isset($this->reader->sheets[0]['cells'][$i][8])){
                $aData['NOTE'] = $this->reader->sheets[0]['cells'][$i][8];
            }
            //pengecekan error 
            $aMandatory = array("PERNR","BEGDA","ENDDA","REMINDER_DATE","REMINDER_TYPE");
        ...
        for ($i = 0; $i < count($aInput); $i++) {
            $this->emp_motask_m->emp_monitoring_new($aInput[$i]);
            $sReturn.="<tr>";
            foreach($aStringHeader as $header){
