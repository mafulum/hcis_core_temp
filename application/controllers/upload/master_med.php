 ...
 * @author Garuda
 */
class master_med extends CI_Controller {
    ...
    function upload() {
        ...
        if (!empty($_FILES['userfile']) && isset($_FILES['userfile']['name']) && !empty($_FILES['userfile']['name'])) {
            ...
            if (empty($sError)) {
                $filename = "m_med_" . date("Ymd_Hi") . '.xls';
                //copy file
    ...
    function load($filename) {
        ...
        $header = 1;
        $col = 1;
        $aStringHeader = array("PERNR", "BEGDA", "SUBTY", "TEXT1");
        ...
        for ($i = 2; $i <= $MAX_ROW; $i++) {
            ...
            $aData['SUBTY'] = $subty = $this->reader->sheets[0]['cells'][$i][3];
        ...
        $sReturn="<table border='1' cellpadding='0' cellspacing='0'u><tr><th>PERNR</th><th>BEGDA</th><th>SUBTY</th><th>TEXT1</th></tr>";
        for ($i = 0; $i < count($aInput); $i++) {
            $a = $aInput[$i];
            $this->employee_m->emp_medical_new($a);
            $sReturn.="<tr><td>".$a['PERNR']."</td><td>".$a['BEGDA']."</td><td>".$a['SUBTY']."</td><td>".$a['TEXT1']."</td></tr>";
        }
        $sReturn.="</table>";
