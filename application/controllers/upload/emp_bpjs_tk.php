 ...
 * @author Garuda
 */
class emp_bpjs_tk extends CI_Controller {
    ...
    function index() {
        ...
        $data = $this->bpjs_tk_m->reff_upload_page();
    ...
    function upload() {
        ...
        if (!empty($_FILES['userfile']) && isset($_FILES['userfile']['name']) && !empty($_FILES['userfile']['name'])) {
            ...
            if (empty($sError)) {
                $filename = "m_emp_bpjs_tk_" . date("Ymd_Hi") . '.xls';
                //copy file
    ...
    function load($filename) {
        ...
        $header = 1;
        $col = 1;
        $aStringHeader = array("PERNR","BEGDA","ENDDA", "BPJSID", "RDATE","FCLTY","NOTE");
        ...
        for ($i = 0; $i < count($aInput); $i++) {
            $this->bpjs_tk_m->personal_bpjs_tk_new($aInput[$i]);
            $sReturn.="<tr>";
            foreach($aStringHeader as $header){
