 ...
 * @author Garuda
 */
class master_address extends CI_Controller {
    ...
    function index() {
        ...
        $data = $this->upload_m->master_emp_address();
    ...
    function upload() {
        ...
        if (!empty($_FILES['userfile']) && isset($_FILES['userfile']['name']) && !empty($_FILES['userfile']['name'])) {
            ...
            if (empty($sError)) {
                $filename = "m_emp_address_" . date("Ymd_Hi") . '.xls';
                //copy file
    ...
    function load($filename) {
        ...
        $header = 1;
        $col = 1;
        $aStringHeader = array("PERNR","BEGDA","ENDDA", "ADDRESS_TYPE", "ADDR","CITY","STATE","COUNTRY");
...
//        }
        ...
        for ($i = 2; $i <= $MAX_ROW; $i++) {
            ...
            $aData['ADDRESS_TYPE'] = $this->reader->sheets[0]['cells'][$i][4];
            $aData['ADDR'] = $this->reader->sheets[0]['cells'][$i][5];
            $aData['CITY'] = null;
            if(isset($this->reader->sheets[0]['cells'][$i][6])){
                $aData['CITY'] = $this->reader->sheets[0]['cells'][$i][6];
            }
            $aData['STATE'] = null;
            if(isset($this->reader->sheets[0]['cells'][$i][7])){
                $aData['STATE'] = $this->reader->sheets[0]['cells'][$i][7];
            }
            $aData['COUNTRY'] = null;
            if(isset($this->reader->sheets[0]['cells'][$i][8])){
                $aData['COUNTRY'] = $this->reader->sheets[0]['cells'][$i][8];
            }
            //pengecekan error 
            $aMandatory = array("PERNR","BEGDA","ENDDA","ADDRESS_TYPE","ADDR");
        ...
        for ($i = 0; $i < count($aInput); $i++) {
            $this->employee_m->personal_data_addr_new($aInput[$i]);
            $sReturn.="<tr>";
            foreach($aStringHeader as $header){
