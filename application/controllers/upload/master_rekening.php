...
class master_rekening extends CI_Controller {
    ...
    function index() {
        ...
        $data = $this->upload_m->master_emp_rekening();
    ...
    function upload() {
        ...
        if (!empty($_FILES['userfile']) && isset($_FILES['userfile']['name']) && !empty($_FILES['userfile']['name'])) {
            ...
            if (empty($sError)) {
                $filename = "m_emp_rekening_" . date("Ymd_Hi") . '.xls';
                //copy file
    ...
    function load($filename) {
        ...
        $header = 1;
        $col = 1;
        $aStringHeader = array("PERNR","BEGDA","ENDDA", "BANK_MID", "BANK_BRANCH","BANK_ACCOUNT","BANK_PAYEE","BANK_CURR","BANK_NOTE");
        $MAX_ROW = $this->reader->sheets[0]['numRows'];
        $this->load->model('pa/bank_m');
        $aBankMID= $this->bank_m->get_master_bank();
        $aMapBankMid = array();
        foreach($aBankMID as $row){
            $aMapBankMid[$row['bank_mid']]=$row['BANK_NAME'];
        }
        $aFlag=array("1","0");
        ...
        for ($i = 2; $i <= $MAX_ROW; $i++) {
            ...
            $aData['BANK_MID'] = $this->reader->sheets[0]['cells'][$i][4];
            $aData['BANK_BRANCH'] = null;
            if(isset($this->reader->sheets[0]['cells'][$i][5])){
                $aData['BANK_BRANCH'] = $this->reader->sheets[0]['cells'][$i][5];
            }
            $aData['BANK_ACCOUNT'] = $this->reader->sheets[0]['cells'][$i][6];
            $aData['BANK_PAYEE'] = $this->reader->sheets[0]['cells'][$i][7];
            $aData['BANK_CURR'] = $this->reader->sheets[0]['cells'][$i][8];
            $aData['BANK_NOTE'] = null;
            if(isset($this->reader->sheets[0]['cells'][$i][9])){
                $aData['BANK_NOTE'] = $this->reader->sheets[0]['cells'][$i][9];
            }
            $aData['BANK_ORDER'] = 1;
            //pengecekan error 
            $aMandatory = array("PERNR","BEGDA","ENDDA","BANK_MID","BANK_ACCOUNT","BANK_PAYEE","BANK_CURR");
            ...
            //SUBTY
            if(!empty($aData['BANK_MID']) && empty($aMapBankMid[$aData['BANK_MID']])){
                $sError.=$aData['BANK_MID'] . " BANK_MID Unknown,please refer for RULES sheet @row $i<br/>";                
            }
        ...
        for ($i = 0; $i < count($aInput); $i++) {
            $this->bank_m->personal_data_bank_new($aInput[$i]);
            $sReturn.="<tr>";
            foreach($aStringHeader as $header){
