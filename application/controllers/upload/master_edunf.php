 ...
 * @author Garuda
 */
class master_edunf extends CI_Controller {
    ...
    function index() {
        ...
        $data = $this->upload_m->master_edunf();
    ...
    function upload() {
        ...
        if (!empty($_FILES['userfile']) && isset($_FILES['userfile']['name']) && !empty($_FILES['userfile']['name'])) {
            ...
            if (empty($sError)) {
                $filename = "m_edunf_" . date("Ymd_Hi") . '.xls';
                //copy file
    ...
    function load($filename) {
        ...
        while (true) {
            ...
            if ($col > 8 && !empty($text) || $text != $aStringHeader[$col - 1]) {
...
//        }
        ...
        $sReturn="<table border='1' cellpadding='0' cellspacing='0'u><tr><th>PERNR</th><th>BEGDA</th><th>ENDDA</th>
            <th>SLART</th><th>AUSBI</th><th>INSTI</th><th>SLTP1</th><th>SLABS</th><th>SLAND</th></tr>";
        for ($i = 0; $i < count($aInput); $i++) {
            ...
            $sReturn.="<tr><td>".$a['PERNR']."</td><td>".$a['BEGDA']."</td><td>".$a['ENDDA']."</td><td>".$a['SLART']."</td><td>".$a['AUSBI']."</td>
                <td>".$a['INSTI']."</td><td>".$a['SLTP1']."</td><td>".$a['SLABS']."</td><td>".$a['SLAND']."</td></tr>";
        }
        $sReturn.="</table>";
